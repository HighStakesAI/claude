...
2. Analyze product pages with `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run render_page.py <URL> --mode auto` and `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run parse_html.py <URL>`
...
4. If DataForSEO credentials available, fetch marketplace data via
   `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_merchant.py`

## Cost Guardrails

Before ANY DataForSEO Merchant API call:
```bash
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_costs.py check <endpoint>
```
...
```bash
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_costs.py log <endpoint> <actual_cost>
```
