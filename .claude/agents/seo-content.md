...
```
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run metadata_template.py --title "<title>" --description "<desc>" --json
```
...
```
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run metadata_template.py --pairs-file metadata.json --json
```
