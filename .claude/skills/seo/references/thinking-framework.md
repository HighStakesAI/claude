...
- Raw HTML + rendered HTML (via `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run render_page.py`)
...
- Core Web Vitals field data from CrUX (via `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run pagespeed_check.py`)
...
- A PDF via `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run google_report.py` when the user asks for one.
