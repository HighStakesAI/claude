...
`"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run <script.py>`. That is the single
...
`"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" setup`, report core and Chromium
status separately, and do not fall back to global or user package installation.
For diagnosis, execute
`"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" doctor --json`; its output
intentionally omits absolute paths and environment values. If any
`"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run` command reports that setup is
...
3. If Google API credentials detected (`"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run google_auth.py --check`), also spawn seo-google agent
...
6. If backlink APIs detected (`"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run backlinks_auth.py --check`), also spawn seo-backlinks agent
...
10. If drift baseline exists for this URL (`"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run drift_history.py <url>`), also spawn seo-drift agent
...
After any analysis command completes, offer to generate a PDF report via `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run google_report.py`.
