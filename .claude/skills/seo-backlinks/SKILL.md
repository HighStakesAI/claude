...
2. **Moz API** (free signup): `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run backlinks_auth.py --check moz --json`
3. **Bing Webmaster** (free signup): `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run backlinks_auth.py --check bing --json`
4. **Keywords Everywhere** (free signup, single-metric): `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run backlinks_auth.py --check keywordseverywhere --json`
...
Run `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run backlinks_auth.py --check --json` to detect all sources at once.
...
**Moz API:** `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run moz_api.py metrics <url> --json` → Domain Authority, Page Authority, Spam Score, linking root domains, external links.

**Keywords Everywhere:** `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run keywordseverywhere_api.py rank <domain> --json` → 0-10 domain rank only (no link counts). Use as a fallback when Moz isn't configured; do not use in place of Moz when both are available.

**Common Crawl:** `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run commoncrawl_graph.py <domain> --json` → PageRank, harmonic centrality, and low-confidence rank/presence data.
...
**Moz API:** `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run moz_api.py anchors <url> --json`

**Bing Webmaster:** `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run bing_webmaster.py links <url> --json` (extract anchor text from link details)
...
**Moz API:** `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run moz_api.py domains <url> --json` → domains with DA scores

**Common Crawl:** `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run commoncrawl_graph.py <domain> --json` → domain-level rank/presence data, no verified referring-domain counts
...
**Moz API:** Raw vendor spam_score from `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run moz_api.py metrics <url> --json` (source-label the value; apply thresholds only if verified against current Moz docs)

**Verification Crawler:** `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run verify_backlinks.py --target <url> --links <file> --json` (verify suspicious links still exist)
...
**Moz API:** `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run moz_api.py pages <domain> --json`
...
**Bing Webmaster:** `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run bing_webmaster.py compare <url1> <url2> --json`
...
**Moz API:** Compare DA/PA between domains via `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run moz_api.py metrics <url> --json` for each
...
**Verification Crawler:** For known links, verify current status with `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run verify_backlinks.py --target <url> --links <file> --json`
...
```bash
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run validate_backlink_report.py --report <report>.json --json
```
