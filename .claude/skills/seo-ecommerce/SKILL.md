...
```
1. "$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run render_page.py <url> --mode auto → raw/rendered HTML
2. "$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run parse_html.py --url <url>   → SEO elements
...
```bash
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_costs.py check merchant_google_products_search
```
...
After each call:
```bash
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_costs.py log merchant_google_products_search <cost>
```
...
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_merchant.py search "<keyword>" --marketplace google

# Seller analysis: merchant ratings and dominance
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_merchant.py sellers "<keyword>"

# Normalize results for analysis
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_normalize.py results.json --module merchant
```
...
```bash
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_costs.py check merchant_amazon_products_search
```
...
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_merchant.py search "<keyword>" --marketplace amazon

# Cross-marketplace comparison
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_merchant.py compare "<keyword>"
```
...
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run ucp_check.py https://store.example.com --json

# With endpoint reachability probes (HEAD each declared capability)
"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run ucp_check.py https://store.example.com --probe-endpoints --json
```
