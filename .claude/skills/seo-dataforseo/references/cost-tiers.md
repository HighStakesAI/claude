...
Configure with: `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_costs.py config --mode threshold --threshold 0.50 --daily-limit 10.00`
...
1. Run `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_costs.py check <endpoint> [--count N]`
...
5. After call completes, log: `"$(git rev-parse --show-toplevel)/.claude/skills/seo/scripts/claude-seo" run dataforseo_costs.py log <endpoint> <cost>`
